<!--
  ...
  Markdown has no colour, so the document's own annotation legend is encoded as text:
    ...
    magenta italic -> trailing `[14 Aug 2026 MOM]` tag
  ...
  v8.0 adds the 14 Aug 2026 client MOM: new IDs SA-22..SA-25, FR-129..FR-139, new
  Sections 4.6 and 9.3, magenta-tagged edits to Sections 3, 4, 5, 7, 8, 9, 10, 14, 15, 16.
  Main themes: the business-owner evidence journey (announcement letter -> dashboard ->
  upload-or-reason), CCO review outcomes, an N/A register, glossary, test cloning,
  spreadsheet-only staff entry, and blue/white branding. Several items confirm earlier
  versions and are marked confirmed rather than rewritten. Resolves TS-04; partially
  advances EV-03 and GAP-06; adds GAP-21.
-->
...
| **Field** | **Value** |
|---|---|
| Document | ControlIQ — Product Requirements Document (PRD v8.0) |
...
| Status | v8.0 — the 14 August 2026 client MOM incorporated. Completes the business-owner side of the records-request workflow (announcement letter → dashboard → upload-or-reason), defines the CCO's three review outcomes, adds an N/A register, a glossary, Super Admin test cloning, an AML news feed source, and spreadsheet-only staff entry. Branding decided as blue and white, exact codes pending. |
| Product name | ControlIQ (renamed from ComplianceIQ, August 2026) |
| Supersedes | v7.0 (August 2026). v4.0 (June 2026) remains the last .docx-signed baseline and was issued under the ComplianceIQ name |
...
> **NOTE** *(blue)*
>
> **🗂 Annotation Legend — added 3 Jul 2026, extended 10 Aug 2026, 12 Aug 2026 and 17 Aug 2026**
...
> - ***Magenta italic text = decision, confirmation or new requirement sourced from the client meeting of 14 August 2026. This is the material that turns v7.0 into v8.0.*** `[14 Aug 2026 MOM]`
...
> - ***New in v8.0: magenta text marks decisions from the 14 August 2026 client meeting. If you read only one thing in this revision, read Section 7.4 — the business owner's side of evidence collection is now specified end to end, from announcement letter to upload-or-reason.*** `[14 Aug 2026 MOM]`

## What Changed in v8.0

***The 14 August 2026 client meeting walked the testing workflow from the business owner's side for the first time, settled how the CCO closes out a test, and added several platform-wide items. Where the minutes confirm v5.0–v7.0 text — audit trails, deactivate-don't-delete, call-tree gap evaluation, test versioning, rule-change alerts — the existing requirement is left standing and marked confirmed. The table below lists what is genuinely new or changed.*** `[14 Aug 2026 MOM]`

| **Change** | **Where** | **Why it matters** |
|---|---|---|
| The test announcement letter goes to a **distribution list** of all relevant business units, auto-populated by the platform; the CCO can add or edit recipients, but only with a recorded justification | Section 7.4 — FR-83 revised, FR-132 | The letter was previously addressed to a single business owner. Editing the auto-populated list is now itself an audited decision |
| Every test shows an overview and the request list needed to execute it | Section 7.2 — FR-129 | The request list stops being a hidden artefact of Section 7.4 and becomes part of the test's own surface |
| Each request list item is mapped to a **business unit** | Section 7.4 — FR-131 | Accountability moves from one owner per test to a unit per item — different items on one test can belong to different units |
| The business owner clicks the link in the announcement letter and lands on a dashboard showing the test and every request item on their side | Section 7.4 — FR-133 | Resolves TS-04. The business side finally has a defined surface; how they authenticate is still EV-03 |
| Against each request item the business owner uploads the document — or, where no document exists, records a reason | Section 7.4 — FR-134 | "We don't have that" becomes recorded compliance information instead of silence |
| Each request item records when it was asked, when it was received, and **who uploaded it** | Section 7.4 — FR-85 revised | The uploader's identity joins the request/receipt dates in the audit trail |
| The Lead Tester sees the AI-identified gaps against each test step and records a status per step | Section 7.5 — FR-135 | Gap review happens inside the step-by-step execution, not as a separate afterthought |
| The Lead Tester submits the finished test for CCO review; the CCO has exactly three outcomes — mark completed, assign a remediation owner, or return it to the Lead Tester. **Only the CCO can assign a remediation owner to a test** | Sections 7.1 and 8.3 — FR-136 | Closes part of GAP-06, but a test-level remediation owner conflicts with the per-milestone owners in FR-41 — logged as GAP-21 |
| Tests marked Not Applicable are shown separately, with reason, who decided, and when | Section 7.2 — FR-130 | The N/A appendix (FR-99) gains an in-platform register counterpart |
| Users are deactivated, never deleted; system logs remain intact | Section 3.3 — FR-14 confirmed | Restates FR-14 and the immutable audit trail — no change, marked confirmed |
| Manual person-by-person staff entry is removed; staff enter the org only by spreadsheet upload | Section 10.1 — FR-62 revised | The dual entry route in FR-62 is cut to one. Onboarding (FR-120) inherits the same change |
| Org chart level changes require the **"Firm Admin"** role | Section 10.2 — FR-139 | Presumed to be the Firm Super Admin system role under the client's name for it — confirmed as open question UM-01 |
| No page displays the org call tree | Section 10.5 — FR-70 note | The call tree is captured and gap-checked but deliberately not visualised. A descope decision, recorded so nobody builds it |
| Rule fetches also save each rule's execution frequency and priority | Section 4.5 — SA-23 | The fetched record now carries the two fields the pool model and SA-10 tagging consume |
| The Super Admin can clone a test as a new one; the platform records the clone history | Section 4.1 — SA-22 | Content reuse without losing provenance — every clone records what it came from |
| The Super Admin has associate admin roles; final confirmation of their actions rests with Sosinna | Section 4.6 — SA-24 | The Portal gains a two-tier admin model. What exactly needs Sosinna's confirmation is AD-01 |
| A glossary and guidance page in the firm application, managed from a Super Admin page in the Portal | Sections 9.3 and 4.6 — FR-137, SA-25 | Net-new content surface, both sides |
| The news feed ingests data and AML alerts from vendor sources such as NICE Actimize's AML pages | Section 9.1 — FR-138 | Extends FR-54, but collides with the official-APIs-only MVP constraint — see NF-01 and RE-05 |
| Sosinna's team decides, before onboarding, whether a firm has all the requirements to be evaluated with ControlIQ | Section 5.2 — SA-18 note | Names the qualification judgement already implicit in the SA-18 status flow |
| Logo and website colour are **blue and white**; exact colour codes to be finalised in the next discussion | Section 14 — CC-02 | First concrete branding decision. Codes pending as BR-01 |
...
**[FR-13]** Every action any user takes is permanently recorded in an audit log — what they did, when, and from which device. This log cannot be altered or deleted by anyone, including administrators. It exists precisely to prove, to a regulator if needed, that the compliance process was followed correctly. ***CONFIRMED on 14 Aug 2026: the client restated that proper audit trails and logging must be implemented throughout the platform — every module, not only the testing workflow. This is a confirmation of FR-13 and NFR-04, not new scope; it is recorded because the client raised it unprompted, which signals audit coverage will be tested at UAT.*** `[14 Aug 2026 MOM]`

**[FR-14]** When a user leaves the firm, an admin deactivates their account. Everything they ever did in the platform stays in the system — tests, findings, uploads, approvals. Nothing is lost. **💬 Sosinna's comment:** *Any reassignment of that user's open work must be documented with reasoning (e.g. termination, transfer) and kept in an immutable audit trail. See GAP-11 in Section 16.* `[Sosinna's Drive comment]` ***CONFIRMED on 14 Aug 2026: the client restated it in the same shape — the platform must offer deactivation, not deletion, and the system logs must remain intact. FR-14 stands unchanged.*** `[14 Aug 2026 MOM]`
...
**[SA-04]** Every change to a test procedure goes through a review step before it is published. Once published, firms are notified. Any firm that is currently mid-way through running that test continues with the version they started — they are not interrupted by the update. ***CONFIRMED on 14 Aug 2026: any change made to a test is versioned and recorded. This is the SA-02 / SA-04 behaviour restated by the client; no change.*** `[14 Aug 2026 MOM]`

**[SA-22]** ***The Platform Super Admin can clone an existing test and create it as a new one — the step-by-step procedure, evidence checklist, sampling guidance and tags are copied into a new test that is then edited independently. The platform records the clone history: which test it was cloned from, which version, by whom and when, so a test's ancestry is always traceable. A clone is a new test, not a new version of the source — it goes through the same SA-04 review-and-publish gate before any firm sees it.*** `[14 Aug 2026 MOM]`
...
**[SA-15]** ***When a regulation the platform already carries is updated — the examples named on 11 Aug were MiCA and DORA — the Platform Super Admin receives a notification on the Portal dashboard and the affected content re-enters the SA-13 draft queue for the same review, edit, tag and publish cycle. An update is not a change alert to be read and dismissed: it re-opens the content pipeline, and the administrator repeats the review process before anything reaches a firm. This dashboard notification is the Portal-side counterpart of the firm-side FR-35 alert; both fire from the same detection event.*** `[11 Aug 2026 MOM]` ***CONFIRMED on 14 Aug 2026: when a rule change is identified in the latest pull, the system sends alerts to the Super Admin and also to the affected firms so they can update their WSPs. This is SA-15 plus FR-35 restated together; no change.*** `[14 Aug 2026 MOM]`

**[SA-23]** ***When a rule is fetched from the feeds, the platform saves its **execution frequency** and **priority** alongside the fields already captured (article, paragraph, effective date, date of download per SA-12). Frequency is what the FR-78 pending-item pool derives its owed-period from; priority pre-populates the SA-10 tag that Sosinna's team confirms during the SA-13 review. Both are stored on the fetched record at ingest so the review pass starts from the feed's own values rather than a blank field.*** `[14 Aug 2026 MOM]`
...
## 4.6 Portal Administration Roles and the Glossary Library

***NEW in v8.0 — from the 14 August 2026 client meeting.*** `[14 Aug 2026 MOM]`

**[SA-24]** ***The Platform Super Admin is supported by **associate admin roles** — additional Portal accounts on Sosinna's team that can work the Portal's queues (content review, enquiries, test building), but whose actions are subject to **final confirmation by Sosinna**. The Portal is therefore two-tier: associates prepare, Sosinna confirms. Every confirmation is recorded with who prepared and who confirmed. Exactly which actions require her confirmation — every publish, or only designated gates such as qualification and publishing — is not yet defined; see AD-01.*** `[14 Aug 2026 MOM]`

**[SA-25]** ***The Portal contains a management page for glossary and guidance items: Sosinna's team creates, edits and retires glossary terms and guidance notes. Published items appear on the firm-side glossary and guidance page (FR-137). Glossary content follows the same review-before-publish spirit as SA-04 — a firm sees an item only once it is published.*** `[14 Aug 2026 MOM]`
...
***The 14 Aug 2026 meeting named the judgement behind the **Qualified** status: Sosinna's team decides whether a firm has all the requirements to be evaluated with ControlIQ before onboarding begins. Qualification is a human decision by Sosinna's team, not a platform-calculated eligibility check — the platform records the decision and its date against the enquiry, nothing more.*** `[14 Aug 2026 MOM]`
...
| ***Staff, reporting lines and call tree*** `[Aug 2026 onboarding flow]` | ***The firm's people, who reports to whom, and each person's next contact in the business continuity call tree — by spreadsheet upload only (manual entry removed in v8.0, see FR-62)*** `[Aug 2026 onboarding flow]` `[14 Aug 2026 MOM]` | ***NEW in v7.0: the org chart and call tree are built during onboarding, not after. The platform reviews them for gaps in the same step. Tests are tagged to business owners drawn from these records (FR-82), so testing cannot start without them. See 5.6.*** `[Aug 2026 onboarding flow]` |
...
**[FR-120]** ***Onboarding captures the firm's staff records, their reporting lines, and each person's next contact in the business continuity call tree. These are the same records described in Section 10 and are created here, by CSV upload against the platform's template (FR-62) ~~or by manual entry~~. Onboarding is where the org chart and call tree first come into existence.*** `[Aug 2026 onboarding flow]` ***REVISED in v8.0: manual entry is removed — the spreadsheet upload is the only route, per the FR-62 revision.*** `[14 Aug 2026 MOM]`

**[FR-121]** ***The platform evaluates the captured data for gaps and shows the firm the result before onboarding completes. Two checks are mandatory and were named by the client: every employee must have a call-tree contact, and every employee must have a reporting manager. The platform lists the specific people failing each check — a count alone is not actionable — and the firm corrects them in place.*** `[Aug 2026 onboarding flow]` ***CONFIRMED on 14 Aug 2026: call trees are submitted during onboarding, the system evaluates whether every contact has a call contact, and identifies any missing call contact. FR-120 and FR-121 restated by the client; no change. Note the deliberate limit added the same day: the call tree is captured and gap-checked but is not displayed on any page — see Section 10.5.*** `[14 Aug 2026 MOM]`
...
| 2a | CCO / Platform | ***NEW in v5.0.*** Before testing begins, the business unit owner receives an **announcement letter** and an **initial request list** — the documents and records the tester will need. The date each item is requested is recorded from this moment. See Section 7.4. `[Aug 2026 client MOM]` <br> ***REVISED in v8.0: the announcement letter goes to a distribution list of all relevant business units, auto-populated by the platform — see FR-132. The letter carries a link that takes each business owner to their own dashboard of request items — see FR-133.*** `[14 Aug 2026 MOM]` |
...
| 3 | Lead Tester | The Lead Tester opens the assigned test and works through it step by step. Each step shows what to do and what evidence to collect. The Lead Tester records their observation at each step and uploads the relevant files. Other team members (for example, the IT manager) can upload specialist evidence to specific steps, but only the Lead Tester records the formal findings. ***REVISED in v8.0: each step also shows the AI-identified gaps relevant to it, and the Lead Tester records a status per step accordingly — see FR-135 in Section 7.5.*** `[14 Aug 2026 MOM]` |
...
| 10 | CCO | The CCO reviews the entire test, all the Findings, and all the remediation plans. The CCO can send it back to the Lead Tester for corrections, or formally approve it. Approval is recorded in the audit log. ***REVISED in v8.0: the Lead Tester formally submits the test for CCO review, and the CCO has exactly three outcomes — mark the test completed, assign a remediation owner, or return it to the Lead Tester. Only the CCO can assign a remediation owner. See FR-136.*** `[14 Aug 2026 MOM]` |
...
**[FR-129]** ***Every test presents an overview and the request list needed for executing it. The overview states what the test is, its regulation and articles, its scope period, priority, assigned tester(s) and business owner; the request list is the Section 7.4 register of documents and records the test needs, shown as part of the test itself rather than only in the separate request workflow. Both the CCO and the assigned tester see this surface; the business owner sees their slice of it through FR-133.*** `[14 Aug 2026 MOM]`

**[FR-130]** ***Tests marked Not Applicable are shown separately from the active test views — a dedicated N/A register listing, for each item: the test, the written justification and justification file (FR-94, FR-107), **who** made the decision, and **when**. The register is the in-platform counterpart of the FR-99 report appendix, so an N/A decision is inspectable at any time, not only at report generation.*** `[14 Aug 2026 MOM]`
...
> - ***Business owner authority (TS-04) — RESOLVED 14 Aug 2026: the business owner reaches a dashboard from the announcement letter link and sees the test together with every request item on their side (FR-133). Visibility is test-plus-own-request-list; the GAP-05 rule still governs anything beyond that, and how the business owner authenticates remains EV-03.*** `[14 Aug 2026 MOM]`
...
**[FR-131]** ***Each request list item is mapped to a **business unit**. One test's request list can span several units — the KYC records to one unit, the system logs to another — and each item is answered by, and its turnaround measured against, the unit it is mapped to. This refines FR-82: the test still has one accountable business owner, but responsibility for producing each individual record sits with the unit the item names. The FR-86 delay analytics report at this per-unit granularity.*** `[14 Aug 2026 MOM]`

**[FR-132]** ***The announcement letter's distribution list is **auto-populated** by the platform from the business units mapped to the test's request items (FR-131) and the FR-82 business owner. The CCO can add or edit recipients before issue, but any addition or edit requires a recorded **justification** — the auto-populated list is the default, and departing from it is an audited decision, in the same spirit as the FR-109 deviation rule.*** `[14 Aug 2026 MOM]`

**[FR-133]** ***When a business owner receives the announcement letter, they click the link in it and are directed to a dashboard showing the test and **all request list items on their side** — each with what was asked, when, its due date and current status. This is the business owner's working surface for the test: they see their slice of the FR-129 view, not the tester's execution record. Resolves TS-04. How the business owner authenticates to reach this dashboard — a login-bearing role or a scoped expiring link — is still the EV-03 decision.*** `[14 Aug 2026 MOM]`

**[FR-134]** ***Against each request item on that dashboard, the business owner uploads the document. Where they cannot produce a document, the platform records the **reason** instead: the item is marked as having no document available, with a mandatory written reason, timestamped and attributed. A no-document declaration is a tracked outcome of the request item — it is visible to the Lead Tester and the CCO, is carried in the request register alongside received items, and does not silently satisfy the evidence checklist.*** `[14 Aug 2026 MOM]`

### Tracking the delay

**[FR-85]** ***Every request item records, at minimum: what was requested, who it was requested from, the **date requested**, the **date evidence was received**, and the elapsed time between the two. These dates are set by the platform, not typed in by a user.*** `[Aug 2026 client MOM]` ***REVISED in v8.0: the record also captures **who uploaded** the evidence — the identity of the person who satisfied the item, alongside the two dates. Confirmed 14 Aug with the dates restated: each request list item records when asked, when received, and who uploaded it.*** `[14 Aug 2026 MOM]`
...
> **CONFIRMED** *(green — agreed 14 Aug 2026)*
>
> **✅ What we have agreed**
> - ***The announcement letter goes out to all relevant business units via a distribution list when the CCO creates the test and sets its start and end dates (FR-83, FR-132).*** `[14 Aug 2026 MOM]`
> - ***Each request list item is mapped to a business unit (FR-131).*** `[14 Aug 2026 MOM]`
> - ***The business owner clicks the link in the letter and lands on a dashboard showing the test and all request items on their side (FR-133). Resolves TS-04.*** `[14 Aug 2026 MOM]`
> - ***Against each request item the business owner uploads the document, or the reason is recorded where no document exists (FR-134).*** `[14 Aug 2026 MOM]`
> - ***Business units upload the required evidence files, which are validated and verified by the AI engine to identify the gaps. Confirms the FR-88 / FR-110 / FR-112 chain unchanged.*** `[14 Aug 2026 MOM]`
> - ***Each request item records when it was asked, when it was received, and who uploaded it (FR-85 revised).*** `[14 Aug 2026 MOM]`
...
**[FR-135]** ***Gap review happens inside test execution, step by step: as the Lead Tester works through each step of the test (Section 7.1, step 3), the AI-identified gaps relevant to that step are shown against it, and the tester records the **status of each step** accordingly. The per-step status is the tester's working verdict on that step in light of the gaps shown; the overall Pass / Fail / Observation result (step 7) and the FR-112 confirm/dismiss decisions still apply on top. Per-step statuses are timestamped, attributed and retained, so the execution record shows not only what was concluded but the state of each step along the way.*** `[14 Aug 2026 MOM]`
...
**[FR-136]** ***When the Lead Tester submits a test for review, the CCO has exactly three outcomes: (a) **mark the test as completed**, (b) **assign a remediation owner**, or (c) **return the test to the Lead Tester** for further work. **Only the CCO can assign a remediation owner to a test** — no other role, including the Lead Tester, can make that assignment. Each outcome is timestamped, attributed and recorded in the audit log.*** `[14 Aug 2026 MOM]`

> **DEVELOPER NOTE** *(grey — new in v8.0)*
>
> **📌 Note**
> - ***FR-136 assigns a remediation owner **to a test**; FR-41 assigns owners per remediation milestone, per Finding, and step 9 of Section 7.1 lets the Lead Tester or CCO create the plan. These models have to be reconciled: is the test-level owner the single accountable owner under whom milestone owners work, or does FR-136 replace per-milestone ownership? And FR-136(c) partially answers GAP-06 — only the CCO assigns — but leaves the rest of GAP-06 (who drafts the plan, pre-approval assignment) open. Logged as GAP-21; resolve with GAP-06.*** `[14 Aug 2026 MOM]`
...
**[FR-138]** ***The news feed also carries data and AML alerts fetched from industry vendor sources — the site named on 14 Aug was NICE Actimize's anti-money-laundering pages (niceactimize.com/anti-money-laundering) — shown alongside the regulator sources already listed in FR-54. Items are labelled with their source so a vendor alert is distinguishable from a regulator publication.*** `[14 Aug 2026 MOM]`

> **DEVELOPER NOTE** *(grey — new in v8.0)*
>
> **📌 Note**
> - ***FR-138 collides with the standing MVP ingestion constraint (Section 4.5): official APIs and RSS feeds only, no custom HTML scraping. NICE Actimize is a commercial vendor site, not a regulator with a public API. Before this is estimated, confirm whether the source offers an RSS feed or licensed API, whether its content licensing permits redisplay inside a commercial product, and whether the manual-curation fallback is acceptable if it does not. Logged as NF-01; resolve together with RE-05, which this is really a concrete instance of.*** `[14 Aug 2026 MOM]`
...
## 9.3 Glossary and Guidance

***NEW in v8.0 — from the 14 August 2026 client meeting.*** `[14 Aug 2026 MOM]`

**[FR-137]** ***The firm application has a page showing the glossary and guidance: plain-language definitions of the regulatory and platform terms used across ControlIQ, and guidance notes on how to use the platform's compliance workflows. The content is authored and managed centrally by Sosinna's team through the Portal's glossary management page (SA-25) — firms read it, they do not edit it. The page is available to every logged-in firm role.*** `[14 Aug 2026 MOM]`
...
**[FR-62]** Staff can be added in bulk using a CSV upload (the platform provides the template, which follows the format of the source compliance spreadsheet). ~~They can also be added one at a time manually.~~ Integration with HR systems like Workday is not in scope for the first version. ***REVISED in v8.0: the option to manually add a person to the organisation is removed — the spreadsheet upload is the only entry route. The struck-through sentence is kept for context under the supersession convention. Corrections to an individual record after upload are still made in place (FR-121 has the firm fixing gaps "in place"); what is removed is creating a new person by hand. This applies wherever staff enter the platform, including the onboarding capture in FR-120.*** `[14 Aug 2026 MOM]`
...
**[FR-139]** ***Org chart level changes — uploading a new staff spreadsheet, changing reporting lines, editing staff records — require the **"Firm Admin"** role. Other firm roles see the org chart and its analytics read-only. The role named in the minutes is "Firm Admin"; this document reads it as the existing Firm Super Admin system role under the client's name for it, since a ninth system role was not proposed — confirmed as open question UM-01.*** `[14 Aug 2026 MOM]`
...
***DESCOPED in v8.0: there is **no page displaying the org call tree**. The call tree is captured (FR-120), gap-checked (FR-70, FR-121) and its gaps and exceptions are reported in the Section 10.6 analytics — but a visual rendering of the tree itself is deliberately not built. This is recorded so the FR-65 interactive org chart is not extended to the call tree by assumption: the org chart (reporting lines) is visualised; the call tree (emergency contacts) is not.*** `[14 Aug 2026 MOM]`
...
| ***Test announcement and initial request list*** `[Aug 2026 client MOM]` | ***Business unit owner (FR-82), copied to the Lead Tester*** `[Aug 2026 client MOM]` ***— v8.0: sent to the auto-populated distribution list of all relevant business units (FR-132)*** `[14 Aug 2026 MOM]` | ***NEW in v5.0: when a test is assigned and the request list is issued, before testing begins (FR-83). The letter carries the link to the business owner's dashboard (FR-133).*** `[Aug 2026 client MOM]` `[14 Aug 2026 MOM]` |
| ***No document available declared*** `[14 Aug 2026 MOM]` | ***Lead Tester and CCO*** `[14 Aug 2026 MOM]` | ***NEW in v8.0: when a business owner records that no document exists for a request item, with the reason (FR-134).*** `[14 Aug 2026 MOM]` |
...
***v8.0 update — the visual identity has its first decision: the logo and website colour are **blue and white**. The exact colour codes will be finalised in the next discussion — tracked as BR-01 in Section 15. This decides the palette, not the branding posture; CC-02's Synergy-product-vs-standalone-brand question remains open. Design work can proceed on a blue/white scheme with the specific values treated as a late-binding token.*** `[14 Aug 2026 MOM]`
...
| ***TS-04*** | ***Testing*** | ***Does a business owner get visibility of the test they are tagged to, or only their request list?*** | ***No*** | ***RESOLVED 14 Aug 2026 — the business owner reaches a dashboard from the announcement letter link and sees the test plus all their request items. See Section 7.4, FR-133*** `[14 Aug 2026 MOM]` |
...
| ***EV-03*** | ***Evidence*** | ***How does a business unit owner without a platform login upload evidence — new login-bearing role, or scoped expiring upload link?*** | ***Yes*** | ***Partial — 14 Aug 2026 confirmed the surface: a dashboard reached from the announcement letter link (FR-133). The authentication mechanism behind that link is still undecided. Still an estimation blocker.*** `[14 Aug 2026 MOM]` |
...
| ***UM-01*** | ***Roles*** | ***Is the "Firm Admin" role required for org chart changes the existing Firm Super Admin system role under another name, or a distinct ninth system role?*** | ***No*** | ***New in v8.0 — see Section 10.2, FR-139. This document assumes Firm Super Admin*** `[14 Aug 2026 MOM]` |
| ***BR-01*** | ***Branding*** | ***Exact colour codes for the blue and white logo and website — to be finalised in the next discussion.*** | ***No*** | ***New in v8.0 — see Section 14, CC-02. Palette decided, values pending*** `[14 Aug 2026 MOM]` |
| ***NF-01*** | ***News feed*** | ***Do vendor AML sources such as NICE Actimize offer an RSS feed or licensed API, and does their content licensing permit redisplay? The MVP constraint forbids HTML scraping.*** | ***No*** | ***New in v8.0 — see Section 9.1, FR-138. Resolve with RE-05*** `[14 Aug 2026 MOM]` |
| ***AD-01*** | ***Portal*** | ***Which associate-admin actions require Sosinna's final confirmation — every publish and qualification, or only designated gates?*** | ***No*** | ***New in v8.0 — see Section 4.6, SA-24*** `[14 Aug 2026 MOM]` |
...
> - ***v8.0 adds four open questions (UM-01, BR-01, NF-01, AD-01) — none of them estimation blockers — and closes TS-04. EV-03 moves to partial: the business owner's surface is now defined (FR-133) but the authentication behind it is not, so EV-03 stays on the blocker list. NF-01 is really a concrete instance of RE-05 and should be resolved in the same session. Running total of open estimation blockers is unchanged from v7.0.*** `[14 Aug 2026 MOM]`
...
| ***GAP-21*** | ***Testing — Remediation*** | ***⚠ FR-136 has the CCO assigning a remediation owner **to a test** as a review outcome; FR-41 assigns owners per remediation milestone, per Finding, and step 9 of Section 7.1 has the plan drafted by the Lead Tester or CCO with owners notified immediately. These models must be reconciled before the remediation data model is built: is the test-level owner a single accountable owner above per-milestone owners, or a replacement for them? What happens to a test with multiple Findings whose fixes belong to different people? And when the CCO chooses "assign a remediation owner" instead of "mark completed", what state is the test in — completed-with-remediation, or held open until remediation closes? Resolve together with the remainder of GAP-06.*** `[14 Aug 2026 MOM]` | ***New in v8.0 — open*** |
...
> - ***v8.0 tally: the 14 Aug meeting largely closed GAP-06 (only the CCO assigns a remediation owner, at review time) and added one — GAP-21, the test-level vs. per-milestone remediation ownership model. Net 21 gaps, 8 resolved or mostly resolved. GAP-21 belongs in the same session as the remainder of GAP-06 and the GAP-14/GAP-15 assignment discussion — all four are about who owns what once a test leaves the tester's hands.*** `[14 Aug 2026 MOM]`
...
> - ***v8.0 status against the freeze clause: Section 18 is still unsigned, so this revision is pre-signature and needs no contract amendment. It is client-initiated — every change originates from the client's own 14 Aug meeting minutes. Version supremacy transfers again: v8.0 is the baseline and v7.0 joins the superseded list. Net-new build to flag before signature: the business-owner dashboard and upload-or-reason flow (FR-133, FR-134), per-unit request mapping and the justified distribution list (FR-131, FR-132), the N/A register (FR-130), per-step gap statuses (FR-135), the glossary both sides (FR-137, SA-25), test cloning (SA-22), associate admin roles (SA-24), and the vendor AML feed (FR-138, pending NF-01).*** `[14 Aug 2026 MOM]`
...
| v12.0 — PRD v8.0 | Aug 2026 | ***Minutes of the 14 August 2026 client meeting incorporated. The business-owner side of the records-request workflow specified end to end: announcement letter to a distribution list of all relevant business units, auto-populated with justified edits (FR-83 revised, FR-132); each request item mapped to a business unit (FR-131); business owner reaches a dashboard from the letter link showing the test and all their request items — resolves TS-04 (FR-133); per-item upload or a recorded no-document reason (FR-134); uploader identity recorded alongside the request/receipt dates (FR-85 revised). Every test shows an overview and its request list (FR-129). Per-step gap visibility and per-step status recording during execution (FR-135). Lead Tester submits for CCO review; the CCO's three outcomes — completed, assign a remediation owner, or return — with remediation owner assignment CCO-only (FR-136, GAP-21). Dedicated N/A register with reason, who and when (FR-130). Manual staff entry removed — spreadsheet upload only (FR-62 revised). Org chart changes gated on the "Firm Admin" role (FR-139, UM-01). Glossary and guidance page firm-side with Portal management (FR-137, SA-25). Vendor AML alerts in the news feed, e.g. NICE Actimize (FR-138, NF-01). Rule fetches save execution frequency and priority (SA-23). Super Admin test cloning with recorded history (SA-22). Associate admin roles with final confirmation by Sosinna (SA-24, AD-01). Pre-onboarding qualification named as Sosinna's team's decision (5.2 note). No page displays the org call tree (descope, 10.5). Branding decided blue and white, codes pending (CC-02, BR-01). Confirmations: platform-wide audit trails (FR-13/NFR-04), deactivate-don't-delete (FR-14), scope dates set by the CCO (FR-80), AI validation of business-unit uploads (FR-88/110/112), test versioning (SA-02/SA-04), call-tree gap evaluation at onboarding (FR-120/121), rule-change alerts to Super Admin and affected firms (SA-15/FR-35). Four new open questions (UM-01, BR-01, NF-01, AD-01), TS-04 resolved, EV-03 partial, one new gap (GAP-21).*** |
...
> - ***v8.0 follows the v6.0 rule: partly a confirmation pass, but it introduces net-new functional scope (the business-owner dashboard, the glossary, test cloning, the N/A register) and revises two previously agreed requirements outright (FR-62 loses manual entry; FR-83's addressee becomes a distribution list), so it is recorded as a version.*** `[14 Aug 2026 MOM]`

# 18. Sign-Off

This document represents the agreed product baseline as of August 2026. Development estimation proceeds from this version — v8.0, not v7.0 and not v4.0. Open questions marked 'Needed before estimation' in Section 15 must be resolved first; v5.0 added six (FO-09, PR-03, TS-01, EV-01, EV-02, EV-03), v6.0 added four (TS-05, TS-07, EV-07, EV-08) while closing EV-04, v7.0 added three more (ON-01, ON-02, ON-03 — all in the payment step), and v8.0 adds none while resolving TS-04 and narrowing EV-03. Workflow gaps in Section 16 must be resolved before the relevant development sprint begins.
...
***v8.0 adds a further block of net-new scope that must be reflected in the fee: the business-owner dashboard and upload-or-reason flow (FR-133, FR-134), per-unit request mapping and the justified announcement distribution list (FR-131, FR-132), the test overview and N/A register views (FR-129, FR-130), per-step gap statuses (FR-135), the CCO review-outcome flow (FR-136), the glossary on both sides (FR-137, SA-25), Super Admin test cloning (SA-22), associate admin roles (SA-24), and the vendor AML news feed source (FR-138 — conditional on NF-01). One removal cuts scope: manual staff entry (FR-62). GAP-21 must be resolved before the remediation sprint.*** `[14 Aug 2026 MOM]`
...
*ControlIQ · Product Requirements Document v8.0 · SayOne Technologies · Confidential · August 2026* `[status note]`
