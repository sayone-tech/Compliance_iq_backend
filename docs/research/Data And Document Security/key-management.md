...
Key management is where residency, isolation, insider risk and record survival converge. **NFR-02 requires that each firm has its own encryption key.** Getting the key architecture right is what makes that requirement a real isolation control; getting it wrong makes the encryption decorative and, worse, puts the six-year non-deletable record store (NFR-07) at risk of becoming unreadable.
...
### Key hierarchy **[PROPOSED, implementing NFR-02]**
...
└── Per-firm key  (firm-<id>)                         ← required by NFR-02
...
- Only the document service role may decrypt, **and only where the request's encryption context carries the matching firm identifier.** This is the mechanism that makes a cross-firm data-mixing bug fail closed (NFR-01).
...
- All principals outside the organisation are denied, as are requests from outside the approved EU region set (NFR-03).
...
Destroying a firm's key would render that firm's evidence, results, reports and audit records permanently unreadable. **PRD §2 and NFR-07 state those records cannot be deleted by anyone, including administrators.** An unreadable record is a deleted record in substance.
...
4. **Crypto-shredding is not adopted as an erasure mechanism.** It is incompatible with NFR-07 on its face. If it is ever to be used, that is a Client decision taken with counsel, recorded against the open erasure question (`regulatory-obligations`, `open-questions` L-3). **[OPEN — LEGAL]**
...
|---|---|---|
| **Key deletion destroys records the PRD says cannot be deleted** | Breach of NFR-07 and PRD §2; for the firm, a MiCA record-keeping failure | Deletion denied for audit, evidence and backup keys; per-firm key deletion blocked by a retention precondition; dual approval; alerting |
| Encryption context not enforced in the key policy | Cross-firm decryption possible with one leaked role — breach of NFR-01 | Policy template with a mandatory firm-identifier condition; automated policy conformance scanning |
...
| Key-service request cost or throttling at scale | Latency against NFR-05; cost surprise | Bounded data-key caching; monitor throttling; benchmark under load |
...
| Key rotation loses old material, making six-year records unreadable | Silent breach of NFR-07 | Use rotation mechanisms that retain prior key versions; assert readability of historical ciphertext in restore verification (`secure-backups`) |

## Trade-offs

- **Managed key service vs. dedicated single-tenant HSM.** A managed service is cheaper, certified, integrated and adequate for NFR-02. A dedicated HSM costs roughly an order of magnitude more and adds real operational burden. Recommendation: managed key service. **[PROPOSED]**
- **Per-firm keys for every firm (mandated) vs. a shared key with encryption context.** NFR-02 removes the choice. The cost is small; treat it as fixed. **[PRD REQUIRED]**
...
|---|---|---|---|
| DD-08-01 | A managed key service in the selected EU region is the root of trust; a distinct key per firm protects that firm's data | **[PRD REQUIRED]** | NFR-02, NFR-03 |
| DD-08-02 | Separate keys, with distinct administrative roles, for audit logs, backups and evidence sealing | **[PROPOSED]** | supports NFR-04 |
| DD-08-03 | Key policies mandate a matching firm identifier in the encryption context, restrict use to named services, and deny non-organisation principals and non-EU regions | **[PROPOSED]** | implements NFR-01, NFR-03 |
...
| DD-08-05 | Automatic annual rotation with prior key versions retained, so ciphertext written at any point in the six-year window remains readable | **[PROPOSED]** | supports NFR-07 |
| DD-08-06 | Deletion denied unconditionally for audit, evidence and backup keys; per-firm key deletion blocked while any record is inside its retention period | **[PRD REQUIRED in effect]** | NFR-07, PRD §2 |
| DD-08-07 | Crypto-shredding is **not** adopted as an erasure mechanism; any future use is a Client decision taken with counsel | **[OPEN — LEGAL]** | conflicts with NFR-07 |
...
**High** — the hierarchy, per-firm keys with encryption-context binding, separation of duties, and the rule that key destruction must not become a back door around NFR-07.

**Medium** — key-service cost and latency at eventual volume; this needs load testing against the NFR-05 performance target.
