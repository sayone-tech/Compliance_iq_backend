...
The PRD does not mention backups. What it does say, and what governs everything here: **records in the protected classes must survive a minimum of six years and cannot be deleted by anyone (NFR-07, PRD §2), all client data stays in EU data centres (NFR-03), and encryption is AES-256 with a per-firm key (NFR-02).** Everything below is **[PROPOSED]** in service of those.
...
- **PRD NFR-07** — the six-year minimum applies to the records themselves. The primary durable copy of protected records is the immutable evidence store (`immutable-evidence-retention`), which is designed for long retention. Backups exist to recover *operational* state, not as the long-term record archive.
...
- **Residency (`data-residency`)** — every backup copy stays in the EU/EEA. A copy to a non-EU location would breach NFR-03.
...
| Object storage — WSP documents and derivatives | Versioning plus copy | Yes for WSP versions (FR-37 requires permanent retention) | Derivatives are rebuildable |
...
| Infrastructure and application code | Version control, with at least one independent remote | History plus protected branches | CC-03: the Client owns this; ensure the Client can obtain it |
...
- The **immutable evidence store** (`immutable-evidence-retention`) is the mechanism that satisfies NFR-07; it is designed for long retention and its records cannot be deleted.
...
Any schedule chosen must **not** be able to undercut NFR-07 for any record class whose only durable copy is in a backup.
...
|---|---|---|
| Ransomware encrypts or deletes production **and** backups | Total, unrecoverable loss; breach of NFR-07 | Separate account with no trust path, immutable retention lock, copies outside the primary failure domain |
| Backup restored into a weakly controlled environment | Uncontrolled copy of all firms' data; breach of NFR-01 | Isolated verification account with production-equivalent controls; automated teardown; access audited |
...
| Backup copy placed outside the EU | Breach of NFR-03 | Policy restricting replication destinations; automated conformance check |
...
| Backup schedule assumed to satisfy NFR-07 | Records lost after backups expire | The evidence store, not backups, is the NFR-07 mechanism; assert this in design review |
...
- **Firm-granular restore vs. full-restore only.** Recommendation: build it — support-cost reduction and an NFR-01 safeguard. **[PROPOSED]**
...
|---|---|---|---|
| DD-21-01 | Backups are written to a dedicated account with no trust path from production; policy denies all deletion operations on vaults and backup keys | **[PROPOSED]** | protects NFR-07 |
| DD-21-02 | Immutable retention lock on the longer-retention vaults; reversible mode on the short-cycle tier | **[PROPOSED]** | — |
| DD-21-03 | The immutable evidence store, not backups, is the mechanism that satisfies the six-year minimum; backups serve operational recovery | **[PROPOSED]** | NFR-07, `immutable-evidence-retention` |
...
| DD-21-07 | Firm-granular point-in-time restore built as a first-class capability | **[PROPOSED]** | protects NFR-01 |
| DD-21-08 | All backup copies remain in EU regions, enforced by policy and verified continuously | **[PROPOSED]** | NFR-03 |
| DD-21-09 | Backups encrypted with a dedicated backup key whose policy denies deletion to all principals | **[PROPOSED]** | NFR-02 |
| DD-21-10 | Restore operations require dual approval, land only in environments with production-equivalent controls, and generate audit events | **[PROPOSED]** | supports FR-13 |
...
**High** — account isolation, immutable retention, the rebuild-rather-than-back-up principle for derived data, automated restore verification, and the separation between operational backups and the NFR-07 evidence store.
