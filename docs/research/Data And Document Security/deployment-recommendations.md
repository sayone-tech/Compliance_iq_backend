...
|---|---|---|
| 1 | **Cloud organisation and account topology** inside the Client-owned account (`reference-cloud-architecture` §1, the confirmed cloud decision) | Account boundaries cannot be introduced later without a migration |
| 2 | **Organisation guardrails**: EU-only regions, no long-lived access keys, no disabling audit or detection services | Every subsequent resource inherits them; this is how the EU residency requirement becomes technical rather than procedural |
| 3 | **Control-plane audit trail across all accounts and regions → write-only log archive with write-once retention** | You cannot recover logs you never collected, and the immutable audit requirement starts here |
...
| 6 | **Key hierarchy and key policy templates** — per-firm, audit, backup, sealing, with deletion denied where `key-management` requires | Encryption context and key policy shape the data model; the encryption requirement is a schema decision, not a later toggle |
| 7 | **Repository with branch protection, signed commits, ownership rules, secret scanning** | Prevents the first secret leak |
| 8 | **Synthetic data fixture factory covering every the accepted evidence file type list file type** | Blocks the "just copy a bit of production data" habit before it starts |
...
Resist building the WSP mapping feature first because it is the interesting part. Without audit logging and record sealing beneath it, a confirmed mapping is not the durable, attributable record the two-person mapping approval requirement and the permanent WSP version history requirement require, and it will be rebuilt.

Note that **both applications** must be delivered — the Firm Application and the Platform Admin Portal (the PRD's two-application description). The Portal is not a phase-two nicety: without it there are no test procedures for firms to run. Its authorisation boundary should be built at the same time as the Firm Application's, not bolted on.
...
Raise these as a resourcing conversation early. They have lead time, and the fixed-fee engagement model (the fixed-price milestone engagement model) means late discovery is expensive for both sides.
...
- **First firm onboarding is a control test.** Run it as a rehearsal with a design-partner firm, with security observing, and capture every friction point. The PRD's onboarding wizard (the PRD's onboarding wizard section) already has a defined shape; the security-relevant parts are firm provisioning and role assignment.
- **Firm provisioning must be fully automated** — key creation, key policy, row-level-security context, storage prefixes, index namespace, retention profile. Manual tenant setup guarantees inconsistency, and inconsistency in tenant isolation is a breach waiting to happen (the tenant isolation requirement).
- **Bulk import of existing evidence** — firms will arrive with years of material. It needs the same quarantine-scan-promote path, rate-limited, with progress reporting and a rollback that does not leave orphaned derivatives. Note that once imported material is attached to a test it becomes non-deletable (the non-deletable retention requirement), so **classification and correctness at import time matter more than usual**.
- **The revenue-source Excel and staff CSV uploads** (the revenue file requirement, the staff import requirement) are structured-file parse paths and belong on the same hardened pipeline, not on a convenience endpoint.
...
| Synchronous audit write on evidence reads | Adds latency to the hot path | p99 impact against the **the performance requirement two-second dashboard target**; adjust the action classification empirically |
| Five-layer authorisation overhead | Policy evaluation per request | p99 authorisation latency, against the performance requirement |
| Concurrency | **the performance requirement promises 100 simultaneous users per firm without degradation** | Load test at that figure with realistic evidence sizes |
| Inference cost per mapping run | Re-runs are automatic on every new WSP version (the PRD's mapping sign-off rules) | Cost per run at realistic manual sizes; total at expected firm count |
| Mapping accuracy | **The 85% bar is a contractual commitment (the PRD's WSP mapping accuracy commitment)** | Measured against the agreed verification vectors before UAT, not during it |
| Storage growth | Six-year non-deletable retention, with video and ZIP admitted by the accepted evidence file type list | Model at multiples of projected volume; use it to set the configurable file-size limit ceiling |
...
**Do not quote an availability, recovery-time or recovery-point figure to a customer until it has been measured in a full test.** The 99.5% figure is a target, and the open uptime-SLA question is still open.
...
| ISO 27001 / SOC 2 Type II | ISMS operating; a control observation window that cannot be compressed | **the certification roadmap item places these on the roadmap; the open certification question leaves the requirement open and the PRD says the timeline is to be agreed with the Client. No date is assumed here.** **[OPEN]** |

## 9. Documents to produce alongside the build

Requested in essentially every enterprise security review. Having them ready shortens due diligence substantially. Note **the IP ownership term**: all of this is the Client's material, and publication is the Client's decision.

- Security overview: sanitised architecture and control summary
- Sanitised threat model summary (`threat-model`)
- Sub-processor list
- The DPA required by the GDPR processor requirement
...
| Enabling irreversible retention before the retention logic is correct | Permanent, unremovable data | Staged rollout per the PRD's onboarding wizard section |
| Deferring audit logging until "later" | Cannot scope the first incident; the immutable audit requirement unmet | Audit event schema before the first data-handling endpoint |
| Building a delete path "for admin convenience" | **Direct breach of the non-deletable retention requirement** | No delete API for protected classes; static-analysis rule; negative tests |
| Quoting an SLA before measuring | Immediate breach of a commitment | Measure twice, then commit; the open uptime-SLA question is still open |
| Building the Remediation Owner or Portal permissions before the scope questions are answered | Rework, or shipping the wrong visibility | Escalate the Remediation Owner view requirement/the Remediation Owner scope gap and the Portal firm-visibility statement/the Portal system settings requirement before those sprints |
...
- [ ] Sub-processor list published; the GDPR processor requirement DPA executed
