...
| PRD ref | Statement |
|---|---|
| NFR-08 | "The platform targets **99.5% availability** (approximately 44 hours per year). Planned maintenance windows are communicated in advance." |
| TI-02 | "Uptime SLA: Is 99.5% availability sufficient, or do clients require 99.9%? — **Still open — estimation blocker**" |
| NFR-07, §2 | Records must survive: minimum six years, non-deletable. **Losing records is a worse outcome than downtime for this product** |
| FR-77 | The platform *stores* firms' own BCP and DR test reports. That is a product feature, not a statement about the platform's own recovery |
...
- **PRD NFR-07** — record survival is a hard requirement independent of any availability target. A recovery design that could lose records breaches it.
...
- **Residency (`data-residency`)** — any recovery location must be in the EU/EEA (NFR-03). **No emergency exceptions.**
...
|---|---|---|
| **1** | Read access to evidence, reports and audit records; authentication | A firm responding to a regulator that cannot retrieve its evidence is in a materially worse position than one that cannot upload a new file. This also protects the NFR-07 promise |
...
| Backup copies to a second EU region, restore on demand | Low–moderate | Protects records against regional loss; restore measured in hours to days | The minimum that protects NFR-07 against a regional event |
...
**No option is selected.** The choice depends on the answer to TI-02 and on the Client's cost tolerance. **[OPEN]** The recommendation this research does make: **whatever is chosen, record copies must exist outside the primary region's failure domain**, because record loss breaches NFR-07 and cannot be remediated. **[PROPOSED]**
...
2. **AI-degraded mode** — the inference path is unavailable; WSP mapping suggestions queue and the UI states clearly that suggestions are delayed. **Everything else works, including manual mapping** — which is possible precisely because FR-31 makes the human the decision-maker.
...
| Ransomware encrypts production **and** backups | Total loss of six-year records — an unrecoverable breach of NFR-07 | Immutable backup vaults, separate backup account, copies outside the primary failure domain, write-once evidence storage |
...
| Recovery targets contracted before being measured | Immediate breach of a commitment | Measure twice before contracting (`deployment-recommendations` §7); TI-02 is still open |
| Recovery environment outside the EU under pressure | Breach of NFR-03 | Region policy denies it technically, not just procedurally |
...
- **Recovery investment vs. the availability target.** A 99.5% target (NFR-08) permits roughly 44 hours of downtime a year and does not on its own justify a warm standby. A 99.9% target would. **TI-02 is unresolved, so the investment question is unresolved.** Recommendation: cost both and put them to the Client together with the measured restore time. **[OPEN]**
- **Record durability vs. service availability.** These are separable, and for this product they should be separated. **Record durability is non-negotiable (NFR-07); service availability is a target with an open number (NFR-08).** Recommendation: fund record durability first — off-region immutable copies — before funding faster service recovery. **[PROPOSED]**
...
|---|---|---|---|
| DD-16-01 | The platform's availability target is 99.5%, with planned maintenance communicated in advance | **[PRD REQUIRED]** | NFR-08 |
| DD-16-02 | Whether the target becomes 99.9%, and what recovery time and recovery point targets apply | **[OPEN]** | TI-02 |
| DD-16-03 | Recovery priority order: record read access and authentication first, then day-to-day work, then reporting and Portal authoring, then internal tooling | **[PROPOSED]** | supports NFR-07 |
| DD-16-04 | Record copies exist outside the primary region's failure domain, in the EU | **[PROPOSED]** | protects NFR-07; NFR-03 |
...
| DD-16-09 | Three degraded modes implemented and tested: read-only, AI-degraded, evidence-only | **[PROPOSED]** | supports NFR-07, FR-31 |
...
**Not determined** — every recovery target, the recovery architecture itself, and the exercise cadence. These follow from TI-02 and from cost decisions that belong to the Client, and this document deliberately proposes no numbers.
