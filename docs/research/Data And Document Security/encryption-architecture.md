...
**the encryption requirement:** *"All data is encrypted at rest… using AES-256… All data moving between the platform and users' browsers is encrypted using TLS 1.3. Each firm has its own encryption key."*
**the PRD's data and retention table (development note):** *"All data encrypted at rest using AES-256 and in transit using TLS 1.3. Evidence files stored in encrypted object storage with per-tenant encryption keys."*
...
- **Bind ciphertext to context.** Additional authenticated data containing the firm identifier and object identifier means a ciphertext moved into another firm's record fails to decrypt. This converts a data-mixing bug into a hard failure rather than a silent leak — a direct defence of the tenant isolation requirement.
...
|---|---|---|
| Symmetric bulk | **AES-256-GCM** | **[PRD REQUIRED]** — The encryption requirement names AES-256. GCM supplies authentication |
| Key wrapping | AES-256-GCM via the managed key service | Envelope pattern |
| Transit, external | **TLS 1.3** | **[PRD REQUIRED]** — The encryption requirement. Disable TLS ≤ 1.1 entirely; TLS 1.2 only if a specific legacy integration forces it, time-limited and recorded |
...
| Password storage | Argon2id (or scrypt) | the phone-based second factor requirement requires email and password plus a phone-based second factor, so password storage is in scope |
| Randomness | OS cryptographic RNG only | Never a seeded PRNG for security purposes |
| Identifier generation | 128-bit from a cryptographic RNG | Opaque and non-enumerable — supports the tenant isolation requirement |

### Layered encryption model **[PROPOSED]**

```
Layer 3  Field-level (application)   Selected high-sensitivity fields — licence numbers, staff
                                     device serials and asset tags (the staff record fields requirement), any national identifiers.
...
Layers 0 and 1 are baseline and cheap. **Layer 2 is what makes the encryption requirement per-firm key a real isolation control** rather than a label: with storage-service encryption alone, the storage service sees plaintext and a single over-broad grant exposes every firm.
...
| Envelope encryption implemented without AAD | Ciphertext substitution across firms undetected — defeats the tenant isolation requirement | AAD mandatory in the module signature; impossible to call without it |
| Layer 2 skipped because storage-service encryption "is enough" | One over-broad grant exposes every firm; the encryption requirement per-firm key becomes decorative | Architecture rule; per-firm key plus per-object data key enforced in the storage service |
...
| Performance regression from field-level encryption | Latency against the performance requirement two-second dashboard target; pressure to disable | Encrypt a short, documented field list; cache decrypted values in memory only; benchmark early against the performance requirement |
| Certificate expiry outage | Availability incident against the availability target | Automated renewal; expiry monitoring well ahead of the date; no manual certificates |
...
|---|---|---|---|
| DD-07-01 | AES-256 encryption at rest for all client data | **[PRD REQUIRED]** | Encryption requirement, the PRD's data and retention table |
| DD-07-02 | TLS 1.3 for all traffic between the platform and users' browsers | **[PRD REQUIRED]** | Encryption requirement |
| DD-07-03 | Each firm has its own encryption key; evidence is stored in encrypted object storage under that key | **[PRD REQUIRED]** | Encryption requirement, the PRD's data and retention table |
...
| DD-07-05 | AES-256-GCM with mandatory AAD binding the firm identifier and object identity; the function signature makes AAD non-optional | **[PROPOSED]** | implements the tenant isolation requirement/the encryption requirement |
| DD-07-06 | Envelope encryption: per-object data key → per-firm key → managed key service. No client data encrypted directly by a key-service key | **[PROPOSED]** | implements the encryption requirement |
| DD-07-07 | Every ciphertext and signature carries an explicit algorithm and version identifier | **[PROPOSED]** | supports the non-deletable retention requirement longevity |
...
**High** — algorithm selection, the envelope pattern, AAD binding, the layered model and crypto-agility priority. These are settled engineering and they implement the encryption requirement directly.

**Medium** — the latency cost of field-level encryption against the performance requirement two-second dashboard target; benchmark before fixing the encrypted-field list.
