...
|---|---|---|---|---|---|
| A-1 | **the open uptime-SLA question** | Uptime SLA: 99.5% or 99.9%? *(PRD: "still open — estimation blocker")* | Determines the recovery architecture and its cost (`disaster-recovery`) | Recovery design; any customer commitment | Build off-domain record copies to protect the non-deletable retention requirement; commit no availability or recovery figure |
| A-2 | **the open certification question** | Are ISO 27001 or SOC 2 Type II required by clients as a condition of signing? *(PRD: open)* | Certification roadmap item puts them on the roadmap with the timeline "to be agreed" | Assurance planning | Operate controls in a way that would support certification later; assume no date |
| A-3 | **the open public-API question** | Public API: first version or later phase? *(PRD: partial, leaning later)* | An API is a new authenticated surface with its own authorisation and rate-limiting model | API security design | Treat as out of MVP scope |
| A-4 | **the open Year-1 scale question** | Expected number of firms and concurrent users in Year 1? *(PRD: partial — roughly 10 platform users per firm, firm total unknown)* | Drives key-service volume, storage growth and monitoring cost | Cost modelling; the performance requirement load testing | Load test at the performance requirement figure of 100 concurrent users per firm |
| A-5 | **the Portal firm-visibility statement / the Portal system settings requirement** | How much firm data may the Platform Admin Portal team see? *(PRD: open; leaning broad, with evidence visibility handled contractually)* | **This is an authorisation boundary. It cannot be built twice cheaply.** | Portal data layer; audit design | Deny by default; expose only the firm list, jurisdiction, services and last-use date stated in the Portal firm-visibility statement |
| A-6 | **the Remediation Owner view requirement vs the Remediation Owner scope gap** | Remediation Owner: own tasks only, or view access to everything? *(PRD explicitly flags the contradiction)* | Determines the permission set for one of the eight system roles | Role permission model | Implement the narrower the Remediation Owner view requirement reading behind a single policy switch |
| A-7 | **the mapping override initiation gap** | Who may initiate a manual override of a WSP mapping? *(PRD: partially open)* | Determines who can change a mapping that two people must then approve | Mapping workflow | Restrict to the CCO pending an answer |
| A-8 | **the deactivation reassignment gap** | Does the platform force-prompt reassignment of a deactivated user's open items? *(PRD: partially open)* | Affects the deactivate-never-delete requirement deactivation flow and its audit trail | Deactivation workflow | Surface open items on deactivation without blocking the deactivation |
| A-9 | **the open onboarding permission question** | Who may complete the onboarding wizard — any Firm Super Admin or the CCO only? *(PRD: open)* | An authorisation question on a high-impact flow | Onboarding permissions | Allow Firm Super Admin, log the actor |
| A-10 | **the open news feed sourcing question** | Regulatory news feed sourcing: manual, automated, or commercial? *(PRD: open)* | Any automated source is an outbound integration needing an egress allowlist entry and a residency review | Egress allowlist | Treat the regulatory monitoring requirement sources (EUR-Lex, EBA, ESMA) as the only allowlisted feeds |
...
| L-3 | **How do GDPR erasure requests interact with the PRD's non-deletability rule?** (the PRD's data and retention table, the non-deletable retention requirement vs GDPR Art. 17) | The PRD says evidence, results, reports and audit records cannot be deleted by anyone. This is a genuine conflict | DPA wording; any deletion capability at all | Implement no deletion path for protected classes; provide a documented refusal path for the controller to use. **Do not adopt crypto-shredding or soft-delete** |
| L-4 | **When, if ever, does retention end after the six-year minimum?** | Non-deletable retention requirement sets a floor and no ceiling; indefinite retention is in tension with storage limitation | Retention service; storage cost model | Retain; build the retention service so a ceiling can be added later without migration |
...
| P-2 | **How is infrastructure provisioned into, and handed over inside, a Client-owned AWS account (the confirmed cloud decision)?** Who holds root credentials and break-glass custody? | Determines the whole privileged-access model | Foundation build; break-glass design | Design for split custody; confirm before go-live |
| P-3 | **Is a second EU region funded** for record copies and/or recovery? | Protects the non-deletable retention requirement against regional loss; costs money | Recovery design | Cost it and present it; assume nothing |
| P-4 | **What is the configured maximum evidence file size (the configurable file-size limit)?** | Drives storage cost across a six-year non-deletable horizon, and the upload denial-of-service surface | Storage cost model; WAF limits | Set a conservative initial value; it is changeable without a release by design |
| P-5 | **What concrete second factor satisfies the phone-based second factor requirement's "verification step on their phone"?** SMS code, authenticator app, or push approval | SMS is subject to SIM-swap, a live risk for crypto-sector staff | Authentication build | Implement authenticator-app TOTP, which satisfies "on their phone" and is the stronger reading |
...
| P-7 | **Does the report distribution requirement send the report as an attachment or as an authenticated link?** | An attachment puts confidential content outside the audited boundary | Notification design | Send a link; confirm with the Client |
...
| P-10 | **Is a coordinated vulnerability disclosure policy published**, given the IP ownership term assigns all material to the Client? | Publication is the Client's call | Disclosure policy | Prepare it; do not publish without approval |
...
| T-2 | Latency of synchronous audit writes on evidence reads, against the performance requirement two-second target | May require reclassifying which actions are synchronous | Implement synchronously; measure; adjust the classification empirically |
| T-3 | Overhead of five authorisation enforcement layers at the performance requirement's 100 concurrent users per firm | Could breach the performance requirement | Benchmark early; evaluate policy locally |
...
| T-6 | Scanning and processing cost for the media formats the accepted evidence file type list admits (video, audio, screen recordings, archives) | Drives the configurable file-size limit ceiling and the pipeline design | Benchmark with a realistic file mix |
...
| E-2 | SayOne is a processor, not a joint controller, for firm data (the GDPR processor requirement) | Joint controllership would change liability and require a different arrangement |
| E-3 | AI-assisted WSP mapping remains advisory, human-confirmed and two-person-approved (the advisory AI mapping requirement, the two-person mapping approval requirement) | Any weakening would raise automated-decision and AI Act questions that do not currently arise |
| E-4 | AWS is the platform, on a Client-owned account (the confirmed cloud decision) | Component choices would change; the control model would not |
| E-5 | No installable or embeddable artefact ships | Would raise CRA scope |
| E-6 | The Platform Admin Portal is delivered in the same build as the Firm Application (the PRD's two-application description, the PRD's baseline-freeze note) | Sequencing and the Portal authorisation boundary would change |
| E-7 | Notifications remain email and in-platform only (the notification channel decision) | Any additional channel is a new egress path and residency question |
| E-8 | No self-service registration exists anywhere, including the marketing site (the invitation-only account requirement, the no self-serve checkout statement) | Self-registration would break the invitation-only access model |
