...
This is the product's core promise, not merely a security control. The PRD's data and retention table opens with it: *"ComplianceIQ is the permanent record of a firm's compliance activity… Everything the platform stores must be kept securely, and most of it cannot be deleted."*

## What the PRD requires

| Record class (the PRD's data and retention table) | Retention | Deletability |
|---|---|---|
| Firm profile | For as long as the firm is a client | — |
| Regulatory requirement IDs and test procedures | **Full history of every version kept forever** | Old versions never deleted (the test procedure versioning requirement) |
| Test executions | Minimum 6 years | — |
| Test results | Minimum 6 years | **Cannot be changed once signed off** — only an amendment on top (the amendment-not-edit requirement) |
| Evidence files | Minimum 6 years | **Cannot be deleted by anyone** |
| Sampling records | Minimum 6 years | Original selection immutably retained alongside any change (the sample-change approval requirement) |
| Findings | Minimum 6 years | — |
| Remediation action items | Minimum 6 years | — |
| Remediation evidence | Minimum 6 years | **Cannot be deleted** |
| Compliance reports | Minimum 6 years | **Cannot be changed after they are issued** (the immutable issued report requirement) |
| Staff records | Employment duration plus retention period | — |
| Audit log | Minimum 6 years | **Cannot be modified by anyone** (the immutable audit requirement, the permanent audit log requirement) |
| Notification log | Minimum 6 years | — |

Plus: **the non-deletable retention requirement** — "All test results, findings, evidence, reports and audit logs are retained for a minimum of six years. No user — including administrators — can delete them." **the permanent WSP version history requirement** — every WSP version and every mapping change kept permanently with complete version history; nothing overwritten. **the Not Applicable immutability requirement** — a recorded Not Applicable status is immutable, timestamped and permanently retained.
...
- **the PRD's data and retention table / the non-deletable retention requirement** — the binding requirement. Six-year minimum, non-deletable.
...
│   ├── created_at, created_by, approved_by[], approval_time     # the two-person mapping approval requirement / the finding closure requirement approvers
...
|---|---|---|
| 1. Write-once object storage | Object retention in a compliance-grade mode, retain-until derived from the retention service | No principal — including the account root — can delete or overwrite before expiry. This is what makes the immutable audit requirement and the non-deletable retention requirement technically true |
...
**Compliance-grade retention mode versus a governance mode that a privileged principal can shorten:** only the former satisfies "not even the system administrators at SayOne" (the immutable audit requirement). The accepted consequence is that a mistake is permanent, which is why the staged rollout in `deployment-recommendations` §5 matters.

External anchoring — daily Merkle roots, qualified timestamps from a trust service provider, a published root feed, a distributable verifier tool — would materially strengthen third-party verifiability. **None of it is required by the PRD, and the verifier tool would also require a Client decision because the IP ownership term assigns all platform code exclusively to the Client.** All **[FUTURE]** (appendix 39).

### Retention service **[PROPOSED, implementing the non-deletable retention requirement]**

```
record_class            → { min_retention, basis, deletable, legal_hold_capable }

audit_log               → 6y minimum , the non-deletable retention requirement / the permanent audit log requirement      , not deletable
notification_log        → 6y minimum , the PRD's data and retention table                  , not deletable
test_execution          → 6y minimum , the PRD's data and retention table                  , not deletable
test_result             → 6y minimum , the PRD's data and retention table, the amendment-not-edit requirement           , not deletable, not modifiable after sign-off
evidence_file           → 6y minimum , the PRD's data and retention table, the PRD's testing workflow step 5     , not deletable
sampling_record         → 6y minimum , the PRD's data and retention table, the sample-change approval requirement          , not deletable
finding                 → 6y minimum , the PRD's data and retention table                  , not deletable
remediation_item        → 6y minimum , the PRD's data and retention table                  , not deletable
remediation_evidence    → 6y minimum , the PRD's data and retention table                  , not deletable
compliance_report       → 6y minimum , the PRD's data and retention table, the immutable issued report requirement           , not deletable, not modifiable after issue
wsp_version + mappings  → permanent  , the permanent WSP version history requirement               , never overwritten
requirement_id_versions → permanent  , the PRD's data and retention table, the test procedure versioning requirement           , old versions never deleted
firm_profile            → client relationship duration, the PRD's data and retention table , —
staff_record            → employment + retention period, the PRD's data and retention table, —
...
| A deletion or modification path exists for a protected class | **Direct breach of the immutable audit requirement / the non-deletable retention requirement** | No delete API; static-analysis rule; tests asserting delete and update fail; write-once storage as the backstop |
...
| Storage cost growth over six-plus years | Unbudgeted cost; pressure to weaken retention | Lifecycle transition to colder storage classes that preserve the retention lock; model cost at multiples of current volume; note the accepted evidence file type list admits video |
...
- **Compliance-grade write-once retention (unbreakable; mistakes permanent) vs. a governance mode (recoverable; defeatable by a privileged insider).** The immutable audit requirement removes the choice for audit and evidence: **compliance-grade**, with a staged rollout and a preview-and-approve step before sealing. **[PRD REQUIRED in effect]**
...
|---|---|---|---|
| DD-15-01 | Test results, findings, evidence, remediation evidence, reports, audit logs and notification logs are retained for a **minimum of six years** | **[PRD REQUIRED]** | Non-deletable retention requirement, the PRD's data and retention table |
| DD-15-02 | None of those records can be deleted by any user, including administrators; no deletion API exists for them | **[PRD REQUIRED]** | Non-deletable retention requirement, the PRD's data and retention table, the PRD's testing workflow step 5 |
| DD-15-03 | A signed-off test result cannot be changed; correction is by amendment with a written explanation, the original retained | **[PRD REQUIRED]** | Amendment-not-edit requirement |
| DD-15-04 | An issued report is permanently archived exactly as issued and cannot be changed | **[PRD REQUIRED]** | Immutable issued report requirement |
| DD-15-05 | Every WSP version and every mapping change is retained permanently with full version history; nothing is overwritten | **[PRD REQUIRED]** | Permanent WSP version history requirement |
| DD-15-06 | Requirement ID and test procedure versions are retained in full; old versions are never deleted | **[PRD REQUIRED]** | the PRD's data and retention table, the test procedure versioning requirement |
| DD-15-07 | Records in protected classes are stored as self-contained sealed packages with content digests, a signed manifest and chain linkage | **[PROPOSED]** | implements the immutable audit requirement |
| DD-15-08 | Write-once object retention in a compliance-grade mode, retain-until derived automatically from the retention service, with a staged rollout | **[PROPOSED]** | implements the immutable audit requirement, the non-deletable retention requirement |
| DD-15-09 | Per-firm hash chain over manifests, with scheduled internal verification of a sample and the chain head | **[PROPOSED]** | implements the immutable audit requirement |
| DD-15-10 | Retention service with per-class minimums and legal-hold capability; extension supported, shortening below the PRD floor impossible | **[PROPOSED]** | implements the non-deletable retention requirement |
...
| DD-15-14 | External anchoring (qualified timestamps, published Merkle roots) and a distributable verifier tool | **[FUTURE]** | not in PRD; the IP ownership term affects publication |
...
**High** — the sealed package format, compliance-grade write-once storage, hash chaining, and the per-class retention service. These implement the PRD's data and retention table and the non-deletable retention requirement directly and are the standard answer for regulated record stores.
