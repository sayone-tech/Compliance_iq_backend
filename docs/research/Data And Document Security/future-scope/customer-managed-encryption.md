...
**The PRD's encryption requirement is the encryption requirement: AES-256 at rest, TLS 1.3 in transit, and a distinct encryption key per firm.** Those keys are platform-managed, inside the Client-owned AWS account (the confirmed cloud decision).
...
- no commercial tiering of security features at all — pricing is seat-based with two plan structures (the pricing model decision), not security-tiered.

Earlier drafts of this research described a three-tier key-custody model (platform-managed / customer-managed / hold-your-own-key) as an accepted architecture and as a commercial differentiator. **That was not supported by the PRD and has been withdrawn.** The per-firm key required by the encryption requirement is implemented as described in `key-management`.

An additional consideration specific to this engagement: **the AWS account is owned solely by the Client (the confirmed cloud decision)**, so the "who can produce plaintext" question already has a different answer than in a conventional vendor-hosted SaaS. Any future customer-key offering would need to be designed against that ownership structure, not against the usual vendor-account assumption.
...
1. Which of the two PRD plan structures (Enterprise or seat-based, the pricing model decision) would carry it, and at what price. **This would be a change to the pricing model decision and would require Client approval.**
2. Whether the firm, Synergy as reseller (the reseller contracting model), or SayOne holds the operational responsibility.
3. Whether it is offered at all, given that the reseller contracting model makes Synergy the contracting party with the CASP firm.

### The hard constraint: the non-deletable retention requirement

**A customer-held key gives the customer the ability to make their own records permanently unreadable.** The PRD states that evidence, results, reports and audit records **cannot be deleted by anyone** and must survive a minimum of six years (the non-deletable retention requirement, the PRD's data and retention table). A customer key revocation or key loss would defeat that requirement, and for the firm it would also be a MiCA Art. 68(9) record-keeping failure.

**Any future customer-key design must resolve this conflict explicitly before it is offered.** Options would include excluding the immutable record classes from customer-key coverage, or obtaining an explicit contractual variation of the non-deletable retention requirement for such firms. Neither can be assumed.
...
**High** that the non-deletable retention requirement conflict above is the decisive design question for any future version of this offering.
