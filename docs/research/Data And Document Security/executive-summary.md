...
|---|---|
| Complete data isolation between firms — multi-tenant from day one | Tenant isolation requirement, the PRD's data and retention table |
| AES-256 at rest, TLS 1.3 in transit, **a distinct encryption key per firm** | Encryption requirement, the PRD's data and retention table |
| All client data in EU data centres | EU residency requirement |
| **AWS, EU-resident data centre, account owned solely by the Client** | Confirmed cloud decision (resolved) |
| Immutable audit log — no modify or delete capability for anyone, including SayOne administrators | Immutable audit requirement, the permanent audit log requirement, the PRD's data and retention table |
| Minimum six-year retention; test results, findings, evidence, remediation evidence, reports and audit logs **cannot be deleted by any user including administrators** | Non-deletable retention requirement, the PRD's data and retention table |
| Signed-off results are amendable but never editable; issued reports are permanently archived exactly as issued; every WSP version and mapping change is kept permanently | Amendment-not-edit requirement, the immutable issued report requirement, the permanent WSP version history requirement, the Not Applicable immutability requirement, the sample-change approval requirement |
| GDPR processor obligations under a DPA | GDPR processor requirement |
| Eight fixed system roles; firm role names map to system roles; invitation-only accounts; phone-based second factor; minimum two Firm Super Admins; deactivate-never-delete | The PRD's roles section, the access requirements |
| Exactly the evidence file types the PRD lists; maximum size is Portal configuration, changeable without a code release | Accepted evidence file type list, the configurable file-size limit |
| AI-assisted WSP-to-rule mapping is **advisory**; a compliance officer confirms or adjusts; two independent senior approvers, never the policy author; reversal follows the same process | Advisory AI mapping requirement, the two-person mapping approval requirement, the mapping reversal requirement, the PRD's mapping sign-off rules |
| **Minimum 85% verified mapping accuracy against pre-defined verification text vectors at UAT** — tuning inside the fixed fee | The PRD's WSP mapping accuracy commitment (accepted 3 Jul 2026) |
| Availability **target** 99.5%; dashboard within two seconds; up to 100 concurrent users per firm | Availability target, the performance requirement |
| ISO 27001 and SOC 2 Type II are roadmap items; **timeline to be agreed with the Client** | Certification roadmap item |
...
|---|---|---|
| **GDPR** | **Binds the platform directly** as processor, under a DPA (the GDPR processor requirement) | Data protection impact assessment; sub-processor disclosure; processor security obligations. **The Art. 17 erasure right versus the PRD's non-deletability rule is a genuine unresolved conflict** — see the PRD's testing section |
...
Applicability determinations for NIS2, the AI Act and CRA are **[OPEN — LEGAL]** (`open-questions`, L-5/L-6/L-7). Assuming a regime applies when it does not adds unbudgeted scope to a fixed-price contract (the fixed-price milestone engagement model).
...
|---|---|---|
| Cross-firm data disclosure (breaches the tenant isolation requirement) | 10 | Catastrophic impact; layering reduces likelihood, not consequence |
| Prompt injection in an uploaded WSP conceals a compliance gap | 9 | No complete technical defence exists. Mitigated in depth by the PRD's own the advisory AI mapping requirement confirmation and the two-person mapping approval requirement two-approver rules |
...
| Records destroyed before six years by deletion, key destruction or misconfiguration | 8 | Directly breaches the non-deletable retention requirement |
...
| P-2 | How is infrastructure provisioned into and handed over inside the Client-owned AWS account (the confirmed cloud decision)? Who holds root and break-glass custody? | Client |
| L-1 | Where will development, support and production administration be performed? | Client |
| L-3 | **How do GDPR erasure requests interact with the PRD's non-deletability rule?** (the PRD's data and retention table, the non-deletable retention requirement vs GDPR Art. 17) | Client + counsel |
| A-5 | How much firm data may the Platform Admin Portal team see? (the Portal firm-visibility statement/the Portal system settings requirement — an authorisation boundary, expensive to build twice) | Client |
| A-6 | Remediation Owner scope — The Remediation Owner view requirement versus the Remediation Owner scope gap, a contradiction **the PRD itself flags** | Client |
| P-5 | What concrete second factor satisfies the phone-based second factor requirement's "verification step on their phone"? | Client |
...
| A-1 | Open uptime-SLA question: is 99.5% sufficient, or is 99.9% required? Determines the entire recovery investment | Client |
...
Two phases inside the MVP, **gated by capability, not by calendar**. `security-roadmap` contains no durations, no headcount and no budget figures — the engagement is a fixed-price milestone contract (the fixed-price milestone engagement model) scoped by the PRD.
