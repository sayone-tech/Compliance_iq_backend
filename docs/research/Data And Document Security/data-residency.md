...
|---|---|
| All client data stored in **EU-based data centres** (the EU residency requirement) | Which EU region or regions **[OPEN]** |
| Cloud provider is **AWS** (the confirmed cloud decision) | Whether a second EU region is used for backup or DR **[OPEN]** |
| The AWS account is **owned solely by the Client**, not SayOne (the confirmed cloud decision) | How infrastructure is provisioned into, and handed over inside, a Client-owned account **[OPEN]** |
| GDPR processor position (the GDPR processor requirement) | Where development, support and administration are physically performed **[OPEN]** — see `cross-border-data-processing` |
| Multi-tenant isolation (the tenant isolation requirement) | Any residency product tiering — none exists in the PRD **[FUTURE]** |
...
- **GDPR** does not mandate EU residency; it mandates lawful transfer (Chapter V) and appropriate security (Art. 32). **The PRD mandates EU residency independently, in the EU residency requirement** — so it is a hard requirement here regardless of the GDPR analysis.
...
- **Third-country access law.** US CLOUD Act exposure via a US-parent provider is the standard objection to hyperscaler residency claims. **The PRD selects AWS (the confirmed cloud decision), so this objection is accepted by the Client at the baseline.** Key-custody responses to it are **[FUTURE]** (appendix 39).

## Recommended architecture

### Region policy **[PROPOSED, implementing the EU residency requirement]**

- **Select an EU region and record the selection as a decision.** Selection criteria, in priority order: presence of every AWS service the design depends on; three availability zones; support for object-lock/WORM storage and KMS features needed for the immutable audit requirement and the non-deletable retention requirement; latency to the customer base (EU CASPs, the PRD's product overview); Client preference. **No region is selected in this research set.** **[OPEN]**
...
### AI inference residency **[PROPOSED, supporting the advisory AI mapping requirement]**

The product's only AI feature is AI-assisted WSP-to-rule mapping (the PRD's WSP mapping accuracy commitment). Requirement, stated as an outcome rather than a product:
...
| Email / notifications | EU sending infrastructure. the report distribution requirement sends reports to distribution lists and the PRD's notifications section sends alerts by email | Report *content* leaving by email is a design decision to review — prefer link-plus-authentication over attachment |
...
| Text extraction / OCR output | In-region; inherits the source document's classification (the single WSP upload requirement requires OCR of scanned PDFs) | |
...
|---|---|---|
| A "boring tier" service silently stores data outside the EU | Breach of the EU residency requirement | Component-by-component residency review recorded in a register; vendor intake gate (`supply-chain-security`) |
...
|---|---|---|---|
| DD-02-01 | All client data, and every derivative of it, resides in EU data centres | **[PRD REQUIRED]** | EU residency requirement |
| DD-02-02 | Cloud provider is AWS, on an account owned solely by the Client | **[PRD REQUIRED]** | Confirmed cloud decision |
...
| DD-02-04 | Region restriction is enforced technically by organisation and resource policies, not by convention; CI fails on non-approved regions | **[PROPOSED]** | implements the EU residency requirement |
| DD-02-05 | Observability, error tracking, email, support tooling and CI are subject to the same residency review as the data plane, each with a named EU-resident implementation in the sub-processor register | **[PROPOSED]** | implements the EU residency requirement |
| DD-02-06 | Extracted text, OCR output, embeddings, search index entries, filenames and document titles are classified at the same sensitivity as the source document | **[PROPOSED]** | implements the tenant isolation requirement/the EU residency requirement |
| DD-02-07 | AI inference for WSP mapping runs EU-resident under no-training, no-retention terms; provider and model are not selected here | **[PROPOSED / OPEN]** | supports the advisory AI mapping requirement |
| DD-02-08 | Portability is maintained as a design discipline (thin provider interfaces, infrastructure as code); no alternative-provider or sovereign offering is in scope | **[PROPOSED]** | IP ownership term context |
